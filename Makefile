...
all:		chmod_generate_map chmod_run_test src/dijkstras

dijkstras:	src/dijkstras.cpp
	$(CXX) $(CXXFLAGS) -o $@ $^

.PHONY:		chmod_generate_map chmod_run_test test test-output test-memory clean

chmod_generate_map:
	chmod 777 generate_map.py

chmod_run_test:
	chmod 777 run_test.py
